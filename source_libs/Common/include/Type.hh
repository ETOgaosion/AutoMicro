...
using namespace iSTA_Proto::Common;

enum class AnalysisMode : int { kMax = 1, kMin = 2, kMaxMin = 3 };
enum class TransType : int { kRise = 1, kFall = 2 };
...
#define NS_TO_FS(delay) (static_cast<int>((delay)*g_ns2fs))
#define FS_TO_NS(delay) ((delay) / static_cast<double>(g_ns2fs))
#define NS_TO_PS(delay) (static_cast<double>((delay)*g_ns2ps))
#define PS_TO_NS(delay) ((delay) / static_cast<double>(g_ns2ps))
#define PS_TO_FS(delay) (static_cast<int>((delay)*g_ps2fs))
...
constexpr bool IsDoubleEqual(double data1, double data2) {
  return std::abs(data1 - data2) < double_precesion;
}
